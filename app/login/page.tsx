...
function LoginForm() {
  ...
  const handleDev = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await api.post<ApiSuccess<LoginResult>>('/v2/auth/login', { email })
      const { accessToken, refreshToken } = (res as ApiSuccess<LoginResult>).data
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleDev} className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Prefix <code className="font-mono">std</code> = student · anything else = lecturer
              </p>
            </form>
          </div>
